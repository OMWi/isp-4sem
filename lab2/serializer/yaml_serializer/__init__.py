...
class YamlSerializer:
    ...
    def loads(self, string):
        ...
        if (len(tokens) > 1 and tokens[0] in ['D', 'T', 'S']
    ...
    def dumps(self, obj, is_dict=True, indent=0):
        if type(obj) == dict:
            ...
            for i, val in enumerate(obj):
                if i == 0:
                    if type(val) == dict:
                        string += (' ' * indent + '- ' + 'D' + ' {}').format(self.dumps(val, type(val) == dict, indent + 2).lstrip())
                    else:
        ...
        if type(obj) == tuple:
            ...
            return ' ' * indent + 'T' + ' ' + self.dumps(list(obj), False, indent).lstrip()

        if type(obj) == set:
            if indent == 0:
                return 'S' + '\n' + self.dumps(list(obj), False, indent).lstrip()
            return ' ' * indent + 'S' + ' ' + self.dumps(list(obj), False, indent).lstrip()
